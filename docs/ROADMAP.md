...
5. **Connected support implemented:** students can ask for durable help from lessons, exercises, and recordings on web/native; staff can acknowledge, respond, and resolve from one queue, promote explainable signals into owned intervention cases, schedule follow-ups, keep private notes, resolve with outcomes, and manage restart/absence recovery plans.
...
8. Recording captions/transcripts/chapters; restart/recovery plans are complete in Connected Experience Phase 4.
9. **Implemented:** explainable objective-evidence views, persisted GitHub check relationships, and source-linked curriculum redo/retry patterns in Connected Experience Phase 5.

### 2026-08-15 Connected experience delivery

The connected-experience implementation stream organizes the existing learning, feedback, support, communication, and cohort capabilities around stable connected records. Phase 1 shipped the cohort-specific Student Workspace and relationship foundations in PR #97. Phase 2 shipped submission/help records, reusable student context, queue-preserving navigation, and source-aware messaging in PR #98. Phase 3 shipped URL-backed cohort workspaces, cohort-contained learner switching, exact read-only enrollment previews, global command/message search, composed student activity, and normalized web/native deep links in PR #99. Phase 4 shipped durable interventions, private staff notes, due follow-ups, outcome tracking, and restart/absence recovery plans across web and native in PR #100. Phase 5 implements explainable objective evidence, persisted GitHub check relationships, and cohort struggle/retry patterns with exact source-record drill-down in PR #101. Web and native responsibilities, privacy boundaries, phased scope, and acceptance criteria are defined in `docs/CONNECTED_EXPERIENCE_PLAN.md`.
...
Voice-to-text is detailed in `docs/VOICE_TO_TEXT_PLAN.md`. Native message drafts and Phase 2 reuse in threads, student/staff help, and grading feedback are complete in code behind the same fail-closed release gate. Web and streaming behavior remain conditional on measured use.
...
Curriculum-authoring release update (2026-09-13): PRs #124–#132 are merged. Native staff can search and inspect curriculum, author modules and lessons, safely preview device drafts, edit learning design, replace hosted video, configure browser runners, and create rich student instructions. The consolidated release gate passes 583 Rails tests, 90 web tests, 355 mobile tests, static analysis, dependency/security checks, Expo Doctor 21/21, iOS/Android production exports, and hands-on iOS simulator QA. Signed build `1.0.0 (21)` was produced from exact merged-main commit `f5f28d94a81b549c7c4c3facc3766afabfcd4c27`, uploaded through EAS submission `4289d148-914b-4f5c-b587-779f72266422`, and is `IN_BETA_TESTING` for internal testers. Public App Review remains intentionally separate from this internal TestFlight release and waits for physical-device acceptance.

Recording-library release update (2026-09-13): PRs #134–#136 are merged. Supported YouTube, Vimeo, Loom, direct, and uploaded recordings now share a first-class cross-platform library; hosted media plays inside the app with progress, help context, touch-first staff management, and a safe original-link fallback. The release gate passes 591 Rails tests, 90 web tests, 378 mobile tests, static analysis, security/dependency checks, Expo Doctor 21/21, iOS/Android production exports, and hands-on iOS playback and authoring QA. Signed build `1.0.0 (22)` was produced from exact merged-main commit `11b825c8b50b4a3b56f70a43c8ffebaef0e29911`; EAS submission `ab62f5fc-5a90-44b8-884a-7d3d9e6425fe` finished successfully, Apple processed the binary, and the build is `IN_BETA_TESTING`. Physical-device acceptance remains; public App Review is intentionally separate.
...
Bulk enrollment/team operations, repository inspection, and dense grading/watch matrices remain deliberate responsive-web workflows. Routine student learning, submissions, recordings, communication, focused staff intervention, and the curriculum changes instructors need away from a desk are native.
