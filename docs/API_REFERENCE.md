...
Creating a submission produces an in-app `submission` notification for active instructors and admins. Grading produces one for the student. Expo delivery deep-links staff to `/staff/submission/:id` and students to `/lesson/:lesson_id`; Web Push opens the equivalent web destination. Web and Expo fanout are isolated from one another and both respect the user's global notification preference.
