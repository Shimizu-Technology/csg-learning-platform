  ...
  const publish = async () => {
    ...
    try {
      ...
      Alert.alert(publishImmediately ? 'Recording published' : 'Draft saved', publishImmediately
        ...
        : `${title.trim()} is private until you publish it from the cohort recording manager.`, [
        { text: 'Done', onPress: () => router.replace('/recordings') },
      ]);
    } catch (error) {
      Alert.alert('Upload did not finish', (error as Error).message);
      setStatus('Upload failed. Your recording was not saved.');
    } finally {
      setUploading(false);
    }
  };
  ...
  return <SafeAreaView edges={['top']} style={styles.safe}>
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
        <View style={styles.hero}><View style={styles.heroIcon}><UploadCloud color={palette.rubySoft} size={24} /></View><Text style={styles.heroTitle}>Upload directly to CSG</Text><Text style={styles.heroCopy}>Choose a class video from this device. Recordings stay private drafts by default, and large files upload in retryable parts.</Text></View>
        ...
        <View style={styles.section}><Text style={styles.label}>4 · STUDENT ACCESS</Text><View style={styles.visibilityCard}><View style={styles.flex}><Text style={styles.visibilityTitle}>Publish immediately</Text><Text style={styles.visibilityCopy}>{publishImmediately ? 'Students get access as soon as the upload finishes.' : 'Recommended: save privately, review it, then publish from the cohort.'}</Text></View><Switch accessibilityLabel="Publish immediately" disabled={uploading} value={publishImmediately} onValueChange={setPublishImmediately} trackColor={{ false: '#363C48', true: '#8B3043' }} thumbColor={publishImmediately ? palette.rubySoft : palette.muted} /></View></View>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={publishImmediately ? 'Upload and publish recording' : 'Upload recording as draft'} disabled={!asset || !cohortId || !title.trim() || uploading} onPress={publish} style={({ pressed }) => [styles.publish, pressed && styles.pressed, (!asset || !cohortId || !title.trim() || uploading) && styles.disabled]}><UploadCloud color={palette.text} size={20} /><Text style={styles.publishText}>{uploading ? 'Uploading… keep this screen open' : publishImmediately ? 'Upload and publish' : 'Upload as draft'}</Text></Pressable>
      </ScrollView>
