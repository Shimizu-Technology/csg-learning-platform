...
import { ArrowLeft, BookmarkPlus, CheckCircle2, CircleDashed, Code2, ExternalLink, FileText, GitBranch, GitPullRequest, Globe2, MessageSquareText, XCircle } from 'lucide-react-native';
    ...
    onError: (error) => {
      if (error instanceof ApiError && error.status === 409) {
        ...
        void query.refetch().then((result) => {
          ...
          setReviewBaseVersion({ submissionId, value: latestVersion });
        });
  ...
  return <SafeAreaView edges={['top']} style={styles.safe}>
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flexRoot}>
      <ScrollView ref={scrollRef} keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
        ...
        {submission.exercise_body && <LearningCard><View style={styles.cardHeading}><FileText color={palette.rubySoft} size={18} /><Text style={styles.cardHeadingText}>Assignment</Text></View><AuthoredContent body={submission.exercise_body} compact /></LearningCard>}
