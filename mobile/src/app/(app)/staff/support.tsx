...
export default function StaffSupportScreen() {
  ...
  async function acknowledge(request: HelpRequest) {
    ...
    try {
      if (auth.demo) {
        queryClient.setQueryData(queryKey, { support_queue: updateDemoRequest(queue!, request.id, { status: 'acknowledged', acknowledged_at: new Date().toISOString() }) });
      } else {
  ...
  async function resolve() {
    ...
    try {
      if (auth.demo) {
        queryClient.setQueryData(queryKey, { support_queue: updateDemoRequest(queue!, resolving.id, { status: 'resolved', staff_response: response.trim(), resolved_at: new Date().toISOString() }) });
      } else {
  ...
  return <SafeAreaView edges={['top']} style={styles.safe}>
    ...
    <ScrollView refreshControl={<RefreshControl refreshing={query.isRefetching} onRefresh={() => void query.refetch()} tintColor={palette.rubySoft} />} contentContainerStyle={styles.content}>
      ...
      <View style={styles.metrics}><Metric value={queue.summary.open_help_count} label="requests" tone="ruby" /><Metric value={queue.summary.active_intervention_count} label="cases" tone="warning" /><Metric value={queue.summary.due_follow_up_count} label="due" tone="danger" /></View>
...
function updateDemoRequest(queue: SupportQueue, id: number, patch: Partial<HelpRequest>) { const update = (item: HelpRequest) => item.id === id ? { ...item, ...patch, updated_at: new Date().toISOString() } : item; const nextHelp = queue.help_requests.map(update); const nextResolved = queue.recently_resolved.map(update); const resolved = nextHelp.find((item) => item.id === id && item.status === 'resolved'); return { ...queue, help_requests: nextHelp.filter((item) => item.status !== 'resolved'), recently_resolved: resolved ? [resolved, ...nextResolved] : nextResolved, summary: { ...queue.summary, open_help_count: nextHelp.filter((item) => item.status === 'open').length, acknowledged_help_count: nextHelp.filter((item) => item.status === 'acknowledged').length, urgent_help_count: nextHelp.filter((item) => item.status !== 'resolved' && item.urgency === 'urgent').length } }; }
