...
import { Activity, AlertTriangle, ArrowLeft, ArrowRight, BookOpen, CheckCircle2, ClipboardCheck, ExternalLink, Film, GitBranch, MessageSquare, RotateCcw, UserRound } from 'lucide-react-native';
...
export default function StaffStudentScreen() {
  ...
  const sharedEvidence = data.progress.learning_evidence_scope.shared_across_enrollments;
  return <SafeAreaView edges={['top']} style={styles.safe}>
    ...
    <ScrollView refreshControl={<RefreshControl refreshing={query.isRefetching} onRefresh={() => void query.refetch()} tintColor={palette.rubySoft} />} contentContainerStyle={styles.content}>
      ...
      {sharedEvidence && <View style={styles.scopeNote}><BookOpen color="#7DA8E8" size={17} /><Text style={styles.scopeText}>Progress and work follow this learner across {data.progress.learning_evidence_scope.enrollment_count} {data.progress.learning_evidence_scope.curriculum_name} enrollments. Messages and cohort actions here stay in {data.progress.cohort.name}.</Text></View>}
