...
import { AlertTriangle, ArrowRight, BadgeCheck, BookOpen, CalendarClock, ClipboardCheck, ExternalLink, LifeBuoy, Megaphone, MessageSquare, RotateCcw, Users, type LucideIcon } from 'lucide-react-native';
  ...
  if (!dashboard || !isStudentDashboard(dashboard)) {
    ...
    return <SafeAreaView edges={['top']} style={styles.safe}><ScrollView refreshControl={<RefreshControl refreshing={query.isRefetching} onRefresh={refresh} tintColor={palette.rubySoft} />} contentContainerStyle={styles.content}>
      ...
      <View style={styles.section}><SectionHeading eyebrow="Highest signal first" title="Needs attention" actionLabel={ungraded ? 'Grading queue' : undefined} onAction={ungraded ? () => router.push('/staff/grading') : undefined} />
        <View style={styles.stack}>{attention.slice(0, 8).map((student) => <LearningCard key={`${student.cohort_id}:${student.user_id}`} onPress={() => router.push({ pathname: '/staff/student/[id]', params: { id: String(student.user_id), cohort_id: String(student.cohort_id) } })} label={`Open ${student.full_name} in ${student.cohort_name}`}><View style={styles.row}><View style={styles.studentAvatar}><Text allowFontScaling={false} style={styles.studentInitials}>{initials(student.full_name)}</Text></View><View style={styles.flex}><View style={styles.studentTop}><Text style={styles.cardTitle}>{student.full_name}</Text><Text style={styles.studentPercent}>{Math.round(student.progress_percentage)}%</Text></View><Text style={styles.cardMeta}>{student.cohort_name} · {student.completed_blocks}/{student.total_blocks} steps</Text><View style={styles.signalRow}>{student.redo_count > 0 && <Signal icon={RotateCcw} label={`${student.redo_count} redo`} tone="warning" />}{student.ungraded_count > 0 && <Signal icon={ClipboardCheck} label={`${student.ungraded_count} review`} tone="ruby" />}{isInactive(student.last_activity_at || student.last_seen_at) && <Signal icon={AlertTriangle} label="Inactive 7d+" tone="quiet" />}</View></View><ArrowRight color={palette.quiet} size={18} /></View></LearningCard>)}</View>
      </View>
      <View style={styles.section}><SectionHeading title="Quick actions" /><View style={[styles.quickGrid, largeText && styles.singleColumn]}><QuickAction icon={LifeBuoy} label="Student support" large={largeText} onPress={() => router.push('/staff/support' as Href)} /><QuickAction icon={ClipboardCheck} label="Grade work" large={largeText} onPress={() => router.push('/staff/grading')} /></View><View style={[styles.quickGrid, largeText && styles.singleColumn]}><QuickAction icon={MessageSquare} label="Messages" large={largeText} onPress={() => router.push('/messages')} /><QuickAction icon={Megaphone} label="Updates" large={largeText} onPress={() => router.push('/updates')} /></View><View style={[styles.quickGrid, largeText && styles.singleColumn]}><QuickAction icon={ExternalLink} label="Full staff tools" large={largeText} onPress={() => void openAuthenticatedWebPage(api, '/admin').catch((error) => Alert.alert('Could not open staff tools', (error as Error).message))} /></View></View>
...
function QuickAction({ icon: Icon, label, onPress, large = false }: { icon: LucideIcon; label: string; onPress: () => void; large?: boolean }) { return <View style={[styles.quickItem, large && styles.quickItemLarge]}><LearningCard onPress={onPress} label={label}><Icon color={palette.rubySoft} size={21} /><Text style={styles.quickLabel}>{label}</Text></LearningCard></View>; }
function MetricCard({ icon: Icon, value, label, tone, large = false }: { icon: LucideIcon; value: number; label: string; tone: 'ruby' | 'warning' | 'success'; large?: boolean }) { const color = tone === 'warning' ? palette.warning : tone === 'success' ? palette.success : palette.rubySoft; return <View style={[styles.metricCard, large && styles.metricCardLarge]}><Icon color={color} size={18} /><Text style={styles.metricNumber}>{value}</Text><Text style={styles.metricCardLabel}>{label}</Text></View>; }
