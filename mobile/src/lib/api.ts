...
export class CsgApi {
  ...
  initiateContentVideoMultipartUpload = (contentBlockId: number | undefined, filename: string, contentType: string, fileSize: number) => this.request<{ s3_key: string; upload_id: string }>('/api/v1/uploads/multipart/initiate', { method: 'POST', body: JSON.stringify({ ...(contentBlockId ? { content_block_id: contentBlockId } : {}), filename, content_type: contentType, file_size: fileSize }) });
