...
export function embeddedMediaHtml(source: MediaSource, initialPosition = source.startSeconds) {
  ...
  const bridge = source.provider === 'youtube'
    ? `<script nonce="csg-player" src="https://www.youtube.com/iframe_api"></script><script nonce="csg-player">var timer=null;var player=null;function emit(player,ended){try{var position=player.getCurrentTime()||0;var duration=player.getDuration()||0;window.ReactNativeWebView.postMessage(JSON.stringify({type:'progress',position:position,duration:duration,playing:player.getPlayerState()===1,ended:!!ended}));}catch(e){}}window.csgSeekTo=function(seconds,play){if(!player)return;player.seekTo(seconds,true);if(play)player.playVideo();};function onYouTubeIframeAPIReady(){player=new YT.Player('player-frame',{events:{onReady:function(){emit(player,false);},onStateChange:function(event){var ended=event.data===YT.PlayerState.ENDED;emit(player,ended);if(event.data===YT.PlayerState.PLAYING&&!timer){timer=setInterval(function(){emit(player,false);},1000);}if(event.data!==YT.PlayerState.PLAYING&&timer){clearInterval(timer);timer=null;}}}});}</script>`
    : source.provider === 'vimeo'
      ? `<script nonce="csg-player" src="https://player.vimeo.com/api/player.js"></script><script nonce="csg-player">var player=new Vimeo.Player(document.getElementById('player-frame'));window.csgSeekTo=function(seconds,play){player.setCurrentTime(seconds).then(function(){if(play)return player.play();});};function emit(data,playing,ended){window.ReactNativeWebView.postMessage(JSON.stringify({type:'progress',position:data.seconds||0,duration:data.duration||0,playing:playing,ended:!!ended}));}player.on('timeupdate',function(data){emit(data,true,false);});player.on('pause',function(data){emit(data,false,false);});player.on('ended',function(data){emit(data,false,true);});</script>`
      : '';
