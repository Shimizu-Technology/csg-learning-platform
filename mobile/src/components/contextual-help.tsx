...
export function ContextualHelp({ cohortId, contextType, contextSource = 'primary', contextId, contextLabel }: ContextualHelpProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (result.created) voiceDraft.markSent(clean);
      setMessage('');
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView automaticallyAdjustKeyboardInsets keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
          {query.isPending && !auth.demo ? <Text style={styles.loading}>Checking your requests…</Text> : active ? <View style={styles.stack}>
            ...
            <View><Text style={styles.fieldLabel}>WHAT HAVE YOU TRIED?</Text><TextInput accessibilityLabel="Describe where you are stuck" value={message} selection={selection} onSelectionChange={(event) => setSelection(event.nativeEvent.selection)} onChangeText={setMessage} maxLength={2000} multiline placeholder="Share the step you reached, what you expected, and what happened instead." placeholderTextColor={palette.quiet} style={styles.messageInput} /><View style={styles.voiceRow}><VoiceDraftButton state={voiceDraft.state} disabled={saving} onPress={() => void voiceDraft.start()} /><Text style={styles.voiceHint}>Speak a draft, then review and edit it before sending.</Text></View><VoiceDraftPanel state={voiceDraft.state} durationMillis={voiceDraft.durationMillis} metering={voiceDraft.metering} error={voiceDraft.error} notice={voiceDraft.notice} hasReview={Boolean(voiceDraft.review)} onStop={() => void voiceDraft.stop()} onCancel={() => void voiceDraft.cancel()} onRetry={voiceDraft.retry} onRestore={voiceDraft.restore} onDismiss={voiceDraft.dismissReview} /><Text style={styles.counter}>{message.length}/2,000</Text></View>
...
const styles = StyleSheet.create({
  ...
  voiceRow: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 6 }, voiceHint: { flex: 1, color: palette.subtle, fontFamily: fonts.regular, fontSize: 11, lineHeight: 16 },
});
