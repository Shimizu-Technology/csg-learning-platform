...
export function ConversationRow({ kind, item, onPress }: { kind: 'channel' | 'dm'; item: ChannelSummary | DirectConversationSummary; onPress: () => void }) {
  ...
  return (
    <Pressable accessibilityRole="button" accessibilityLabel={`Open ${title}`} onPress={onPress} style={({ pressed }) => [styles.row, pressed && styles.pressed]}>
      ...
      {item.unread_count > 0 ? <View style={styles.badge}><Text maxFontSizeMultiplier={fontScaleLimits.utility} style={styles.badgeText}>{item.unread_count > 99 ? '99+' : item.unread_count}</Text></View> : <ChevronRight color={palette.quiet} size={18} />}
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  workspace: { ...typography.meta, color: palette.subtle, fontFamily: fonts.medium, textTransform: 'uppercase', letterSpacing: 0.7 },
  badge: { minWidth: 24, minHeight: 24, paddingHorizontal: 7, paddingVertical: 2, borderRadius: 12, backgroundColor: palette.ruby, alignItems: 'center', justifyContent: 'center' },
  badgeText: { ...typography.label, color: 'white', fontFamily: fonts.bold },
});
