...
export function ExerciseEditorModal({ visible, module, defaultWeek, onClose, onSave }: ExerciseEditorProps) {
  ...
  const submit = async () => {
    ...
    if (!instructions.trim()) { setError('Add a starting prompt so the new lesson is useful immediately.'); return; }
    setSaving(true); setError(null);
    try {
  ...
  return <EditorSheet visible={visible} title="New lesson" subtitle={module ? `Add a useful starting point to ${module.name}.` : 'Choose a module first.'} onShow={reset} onClose={onClose} saving={saving} onSave={() => void submit()} saveLabel="Create and continue" saveButtonText="Create">
    ...
    <Field label="STARTING PROMPT"><TextInput accessibilityLabel="New lesson instructions" value={instructions} onChangeText={setInstructions} multiline textAlignVertical="top" placeholder="Explain the goal and what done looks like…" placeholderTextColor={palette.quiet} style={[styles.input, styles.instructions]} /></Field>
  </EditorSheet>;
}
