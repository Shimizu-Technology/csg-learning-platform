...
import type { LessonContentBlock, LessonDetail, SubmissionInput } from '@/lib/types';
...
export function LessonContentBlockCard({ block, lesson }: LessonContentBlockProps) {
  ...
  const openVideo = async () => {
    try {
      if (block.has_s3_video) {
        const result = await api.contentVideoStream(block.id);
        await openExternalPage(result.stream_url);
      } else await openExternalPage(block.video_url);
    } catch (error) { Alert.alert('Video unavailable', (error as Error).message); }
  };
