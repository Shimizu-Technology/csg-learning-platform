...
export function ErrorState({ message, retry, title = 'Couldn’t load this', retryLabel = 'Try again' }: { message: string; retry?: () => void; title?: string; retryLabel?: string }) {
  return <View style={styles.empty}><AlertCircle color={palette.warning} size={28} /><Text style={styles.title}>{title}</Text><Text style={styles.muted}>{message}</Text>{retry && <Pressable accessibilityRole="button" accessibilityLabel={retryLabel} onPress={retry} style={styles.button}><Text style={styles.buttonText}>{retryLabel}</Text></Pressable>}</View>;
}
