...
export function VoiceDraftButton({ state, disabled, onPress }: VoiceDraftButtonProps) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel="Dictate a message draft" accessibilityHint="Records temporary audio and inserts editable text; it never sends automatically" accessibilityState={{ disabled: disabled || busy, busy }} disabled={disabled || busy} onPress={onPress} style={({ pressed }) => [styles.iconButton, (disabled || busy) && styles.disabled, pressed && styles.pressed]}><Mic color={palette.muted} size={19} /></Pressable>;
}
