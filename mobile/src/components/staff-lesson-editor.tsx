import { AlertCircle, ArrowLeft, Check, Cloud, Eye, FileCode2, Film, Pencil, RefreshCw, Save, Trash2 } from 'lucide-react-native';
...
import { StaffCodeRunnerSettings } from '@/components/staff-code-runner-settings';
...
import { StaffVideoSourceEditor } from '@/components/staff-video-source-editor';
...
import type { VideoUploadAsset } from '@/lib/video-upload';
...
interface StaffLessonEditorProps {
  ...
  onUploadVideo: (asset: VideoUploadAsset, contentBlockId: number | undefined, onProgress: (percent: number, label: string) => void) => Promise<{ s3_video_key: string; s3_video_content_type: string; s3_video_size: number }>;
  onAbandonVideo: (s3Key: string) => Promise<void>;
  onScheduleVideoCleanup: (s3Key: string) => Promise<void>;
  onRetryVideoCleanups: () => Promise<void>;
}
...
export function StaffLessonEditor({ lesson, userId, onBack, onSave, onReload, objectiveCatalog = [], rubricCatalog = [], catalogLoading = false, catalogError = false, canCreateResources = false, onRetryCatalogs = () => undefined, onCreateObjective, onCreateRubric, onUploadVideo, onAbandonVideo, onScheduleVideoCleanup, onRetryVideoCleanups }: StaffLessonEditorProps) {
  ...
  useEffect(() => {
    ...
    void loadLessonEditorDraft(userId, initialLesson.id).then(async (draft) => {
      ...
      const restoredFields = draft ? {
          ...
          s3_video_key: draft.s3_video_key !== undefined ? draft.s3_video_key : initialFields.s3_video_key,
          s3_video_content_type: draft.s3_video_content_type !== undefined ? draft.s3_video_content_type : initialFields.s3_video_content_type,
          s3_video_size: draft.s3_video_size !== undefined ? draft.s3_video_size : initialFields.s3_video_size,
          filename: draft.filename,
          instructions: draft.instructions,
          solution: draft.solution,
          submission_type: draft.submission_type,
          runner: draft.runner || initialFields.runner,
  ...
  const updateVideo = (patch: Partial<Pick<LessonEditorFields, 'video_url' | 's3_video_key' | 's3_video_content_type' | 's3_video_size'>>) => {
    setFields((current) => ({ ...current, ...patch }));
    setError(null);
    setNotice(null);
  };

  const updateSubmissionType = (submissionType: LessonEditorFields['submission_type']) => {
    setFields((current) => ({
      ...current,
      submission_type: submissionType,
      runner: submissionType === 'text_submission' ? current.runner : { ...current.runner, enabled: false },
    }));
    setError(null);
    setNotice(null);
  };
  ...
  const showMode = (nextMode: 'edit' | 'preview') => {
    if (nextMode === 'preview' && videoUploading) { Alert.alert('Video upload in progress', 'Keep this editor open until the video is ready to preview.'); return; }
    setMode(nextMode);
    requestAnimationFrame(() => scrollRef.current?.scrollTo({ y: 0, animated: false }));
  };

  const leave = () => {
    if (videoUploading) { Alert.alert('Video upload in progress', 'Keep this screen open until the video is ready to save.'); return; }
  ...
  const save = async () => {
    if (videoUploading) { setError('Wait for the video upload to finish before saving.'); return; }
  ...
  const discard = () => Alert.alert('Discard this draft?', 'This removes the device copy and loads the latest saved lesson.', [
    ...
    { text: 'Discard draft', style: 'destructive', onPress: () => void (async () => {
      setSaving(true); setError(null);
      try {
        const stagedVideoKey = fields.s3_video_key && fields.s3_video_key !== serverFields.s3_video_key ? fields.s3_video_key : null;
        const latest = await onReload();
        if (stagedVideoKey) await onScheduleVideoCleanup(stagedVideoKey);
        await clearLessonEditorDraft(userId, lesson.id);
        ...
        setDraftStatus('saved');
        if (stagedVideoKey) void onRetryVideoCleanups().catch(() => undefined);
      } catch (reloadError) {
        setError((reloadError as Error).message || 'The latest lesson could not be loaded.');
  ...
  return <SafeAreaView edges={['top', 'bottom']} style={styles.safe}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.safe}>
    ...
    <ScrollView ref={scrollRef} automaticallyAdjustKeyboardInsets keyboardDismissMode="interactive" keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
      ...
      {error && <Notice icon={<AlertCircle color={palette.rubySoft} size={18} />} tone="danger" title="Something needs attention" copy={error} />}
      ...
      {mode === 'preview' ? <DraftPreview lesson={previewLesson} dirty={dirty} /> : <>
        ...
        <EditorSection eyebrow="2 · WATCH" title="Lesson video" copy="Use a video link or upload the lesson file directly to Code School.">
          <StaffVideoSourceEditor
            value={{ video_url: fields.video_url, s3_video_key: fields.s3_video_key, s3_video_content_type: fields.s3_video_content_type, s3_video_size: fields.s3_video_size }}
            persistedS3Key={videoBlock?.s3_video_key || null}
            uploadedAt={videoBlock?.s3_video_uploaded_at}
            uploadedBy={videoBlock?.s3_video_uploaded_by}
            onChange={updateVideo}
            onUpload={(asset, onProgress) => onUploadVideo(asset, videoBlock?.id, onProgress)}
            onAbandon={onAbandonVideo}
            onCleanupDeferred={onScheduleVideoCleanup}
        ...
        <EditorSection eyebrow="3 · BUILD" title="Exercise and submission" copy="Write the student-facing task first, then keep the answer in the instructor-only solution.">
          <Field label="INSTRUCTIONS"><StaffRichTextEditor value={fields.instructions} onChange={(value) => update('instructions', value)} /></Field>
        ...
        />
        <View style={styles.actions}><Pressable accessibilityRole="button" accessibilityLabel="Save lesson changes" disabled={!dirty || saving || videoUploading || !draftReady} onPress={() => void save()} style={[styles.primary, (!dirty || saving || videoUploading || !draftReady) && styles.disabled]}><Save color={palette.text} size={19} /><Text style={styles.primaryText}>{videoUploading ? 'Uploading video…' : saving ? 'Saving lesson…' : dirty ? 'Save lesson changes' : 'Everything is saved'}</Text></Pressable>{dirty && <Pressable accessibilityRole="button" disabled={saving || videoUploading} onPress={discard} style={styles.discard}><Trash2 color={palette.muted} size={17} /><Text style={styles.discardText}>Discard device draft</Text></Pressable>}</View>
      </>}
    </ScrollView>
