...
describe('video segments', () => {
  const metadata = {
    video_segments: [
      { label: 'Optional context', start_seconds: 10.9, end_seconds: 20.8, required: false },
  ...
  it('prefers an explicit deep link, then saved progress, then the authored start', () => {
    ...
    expect(playbackStart(segments, 44, 0)).toBe(44)
    expect(playbackStart(segments, 0, 0)).toBe(30)
  })
