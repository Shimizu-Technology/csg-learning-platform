...
export function VideoSegmentEditor({ value, onChange }: { value: VideoSegment[]; onChange: (segments: VideoSegment[]) => void }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      ...
      <button type="button" onClick={() => onChange([...value, { label: '', start_seconds: 0, end_seconds: 60, required: true }])} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white"><Plus className="h-4 w-4" />Add section</button>
    </div>
    <div className="mt-4 space-y-3">
      {value.map((segment, index) => <div key={index} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-3 lg:grid-cols-[minmax(0,1fr)_8rem_8rem_auto_auto] lg:items-end">
        ...
        <button type="button" aria-label={`Remove section ${index + 1}`} onClick={() => onChange(value.filter((_, candidate) => candidate !== index))} className="flex h-11 w-11 items-center justify-center rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
      </div>)}
